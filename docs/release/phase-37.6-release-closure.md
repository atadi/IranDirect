...
| GATE-PROD-ES256 Production ES256 metadata trust | Release signing/verify | **PASS** | Real `pv-meta-prod-2026-01` P-256 key provisioned (DPAPI store, never committed); public anchor embedded in client (`BuiltInReleaseTrust`, `ForProduction`, `allowUnsigned=false`); sign→verify proven with the real key; staging key isolated from production trust; 10 deterministic trust tests + 38 from 37.5 green. See `production-metadata-trust.md` | portable backup into KeePassXC (user action pending) |

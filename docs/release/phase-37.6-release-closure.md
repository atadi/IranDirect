...
| GATE-PROD-ES256 Production ES256 metadata trust | Release signing/verify | **PASS** | Real `pv-meta-prod-2026-01` P-256 key provisioned (DPAPI store, never committed); public anchor embedded in client (`BuiltInReleaseTrust`, `ForProduction`, `allowUnsigned=false`); sign→verify proven with the real key; staging key isolated from production trust; 10 deterministic trust tests + 38 from 37.5 green. Portable PKCS#12/PFX recovery backup written and round-trip verified. See `production-metadata-trust.md` | — |
| GATE-AUTH-CODE Production Authenticode | Windows code-signing | **EXTERNAL BLOCKER** | Pipeline engineered & fail-closed (sign→verify→hash→manifest order, provider abstraction incl. HSM/Azure, signature-seam verifier, readiness command, 7 trust tests). No publicly trusted certificate obtainable by an individual resident in Iran: Microsoft/SSL.com/Sectigo/DigiCert/Entrust embargo Iran; GlobalSign + EU CAs are org-only. Research in `authenticode-provider-feasibility.md`. Ships ES256 + SHA-256 verified downloads only. | publicly trusted code-signing cert for an individual in Iran |
...
`docs/release/cloudflare-r2-certification.md`. The production ES256 metadata trust
root was subsequently provisioned and embedded — see
`docs/release/production-metadata-trust.md`. Every other gate above remains

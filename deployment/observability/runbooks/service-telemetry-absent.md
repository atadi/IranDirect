# Runbook: PathVeerServiceTelemetryAbsent

- **Alert:** `PathVeerServiceTelemetryAbsent`
- **Severity:** warning
- **Dashboard:** `pathveer-service-overview`
- **Prometheus query:** `sum(pathveer:runtime_cycle_rate5m) < 0.0001 and max(timestamp(pathveer_runtime_cycles_started_total)) < time() - 900`
...
## Dashboard
Open the `pathveer-service-overview` dashboard in the PathVeer folder (Grafana).
Prometheus query: `sum(pathveer:runtime_cycle_rate5m) < 0.0001 and max(timestamp(pathveer_runtime_cycles_started_total)) < time() - 900`
...
## Safe checks
Is the PathVeer.Service process running (Services console)? Is Observability:Enabled true? Check Service event log for startup faults. Confirm collector is up.
...
## Related alerts
PathVeerCollectorUnavailable, PathVeerRuntimeCycleFailureRateHigh

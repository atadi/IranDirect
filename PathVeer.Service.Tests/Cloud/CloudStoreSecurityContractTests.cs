...
/// <summary>
/// Contract tests for the JsonStore security-callback boundary and for
/// treating an attacker-planted "Connected" registration as unusable.
///
/// The actual filesystem persistence of the canonical SYSTEM-owned descriptor
/// is proven by the separate LocalSystem integration tool
/// (tools/certification/Test-PathVeerCloudStateSecurity.ps1). These unit
/// tests verify the CALLBACK CONTRACT (distinct directory/file callbacks at
/// the correct lifecycle points) and the USABILITY SEMANTICS without
/// requiring LocalSystem/SeTakeOwnershipPrivilege on the runner.
/// </summary>
public sealed class CloudStoreSecurityContractTests
{
    ...
    public async Task JsonStore_InvokesDistinctDirectoryAndFileCallbacks()
    {
        ...
        // canonical file hardening; here we use recording stubs so the test
        // runs under any runner (no SYSTEM-owner persistence required).

...
function Get-GuestJeaCertificationBoundary([System.Management.Automation.Runspaces.PSSession]$Session,
                                      [System.Management.Automation.Runspaces.PSSession]$JeaSession) {
    # The boundary function now ALSO returns the read-only certification-baseline readiness fields
    # (installerPresent / payloadPresent / basename checks / baselineReady). Reusing this existing
    # privileged function keeps the JEA surface at exactly 12 VisibleFunctions.
    return Invoke-GuestJeaFunction -Session $Session -JeaSession $JeaSession -Function 'Get-PathVeerCertificationBoundary'
}
